...
export async function GET(
  ...
  try {
    ...
    const { data: client, error: clientError } = await supabaseAdminClient
      .from("clients")
      .select(
        "id, firm_id, type, name, full_name, father_name, representation, representative_details, organization_name, email, phone, address, city, province, country, cnic, notes",
      )
      .eq("id", clientId)
      .maybeSingle();
