    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold">{item.clientName ?? "Unassigned client"}</p>
        ...
        <Badge
          variant="outline"
          className="shrink-0 border-white/10 bg-white/5 text-[10px] text-slate-200"
        >
          {item.matterType}
        </Badge>
      </div>
      <div className="mt-2 flex items-center justify-between gap-2">
        <p className="truncate text-[11px] text-slate-300/70">{item.district ? item.district : "—"}</p>
        <Link
          href={`/cases/${item.id}`}
...
function KanbanColumn({ stage, items }: { stage: (typeof STAGES)[number]; items: CaseItem[] }) {
...
export function MatterKanbanBoard({ cases }: { cases: CaseItem[] }) {
  ...
  const itemsFiltered = useMemo(() => {
    ...
    const matchesQuery = (m: CaseItem) =>
      !q ||
      [m.serialNumber, m.caseNumber ?? "", m.clientName ?? "", m.courtName ?? "", m.district ?? ""]
        .join(" ")
        .toLowerCase()
        .includes(q);
  ...
  const byStage = useMemo(() => {
    ...
    for (const k of Object.keys(map) as KanbanStage[]) {
      map[k] = map[k].slice().sort((a, b) => (a.serialNumber || "").localeCompare(b.serialNumber || ""));
    }
    return map;
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    const destStage: KanbanStage | null = stageKeys.has(overId as KanbanStage)
      ...
      : (optimistic[overId] ?? stageForStatus(cases.find((c) => c.id === overId)?.status ?? "pending"));

    if (!destStage) return;

    const currentStage =
      optimistic[activeId] ?? stageForStatus(cases.find((c) => c.id === activeId)?.status ?? "pending");
    ...
    <div className="rounded-[28px] border border-white/10 bg-linear-to-b from-slate-950/90 to-slate-900/90 p-4 text-slate-100 shadow-[0_20px_60px_rgba(2,6,23,0.35)] backdrop-blur-xl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-slate-300/80">Drag matters through NEW → FILED → HEARING → CLOSED.</p>
        </div>
        ...
        <DndContext sensors={sensors} onDragEnd={onDragEnd}>
          <div className="flex gap-3 overflow-x-auto pb-2">
            {STAGES.filter((s) => activeStage === "ALL" || activeStage === s.key).map((stage) => (
              <KanbanColumn key={stage.key} stage={stage} items={byStage[stage.key]} />
            ))}
          </div>
        </DndContext>
        {isPending ? (
