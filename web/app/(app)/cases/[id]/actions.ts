...
export async function updateMatter(values: UpdateMatterFormValues): Promise<ActionState> {
  ...
  if (newlyAssigned.length > 0) {
    ...
    // For each newly assigned user, insert or update the assignment
    for (const userId of newlyAssigned) {
      // First, try to update if record exists (was previously inactive)
      ...
      if (existing) {
        ...
        await (supabase as any).from("matter_assignments").insert({
